...
class FlightController extends Controller
{
	...
	public function flightList(Request $request) 
    {
		...
		if($request->has('dep')){
			...
			$datedeparture =request()->get('dep');
			$date  = request()->get('dep');
			$timestamp = strtotime($date);
			$datedeparture = date("Y-m-d", $timestamp);

		}
		$returnDate ='';
		if($request->has('ret')){
			// $returnDate = Carbon::createFromFormat('d/m/Y',request()->get('ret'))->format('Y-m-d');
			$date  = request()->get('ret');
			$timestamp = strtotime($date);
		...
		return view('Agent.flights.international', compact(['flightresult','destin','datedeparture','calenderresult'])); 
		...
		$result_data .=	  '"AddressLine1": "123 test",
			  "AddressLine2": "",
				...
				if($resultdata->Response->Results->IsGSTMandatory == true && $ics == 0){
					...
					"GSTCompanyName": "ZAP BOOKING PRIVATE LIMITED",
...
//Decode pdf content
			...
			// echo json_encode(array('success' =>false,'message'=>$err));
			 ...
			 // echo json_encode(array('success' =>true,'message'=>$response));
				...
				/*SMS */
				$pd = 1;
				$message = "Dear Admin, '{name}' have successfully booked a flight ticket from {source} to {destination}. PNR is '{PNR}' and ZAP Booking Id is '{BookingID}'. {travel_date} and {flight_name} For more bookings please visit {URL}. Thank you.";
...
$nonlcc = true;
	...
	public function BookingTicket(Request $request, $id = Null){
		if(isset($id) && !empty($id)) 
			...
			{
				//return Redirect::to('/contact')->with('error', Config::get('constants.unauthorized'));
			}
	}
	...
	public function viewTicket(Request $request){
		if(isset($_GET['ticket']) && isset($_GET['email'])){
			...
			$query->whereHas('user', function ($q) use($email){
				...
				{
					//return Redirect::to('/contact')->with('error', 'Ticket Not Exist');
				}	
		}	
	}
	   ...
	   return response()->json(['succcess'=>false,'message'=>'Please login'],401);
		...
		public function flightReview(){
			...
			if($userFlight){
				...
				return view('Agent.flights.review',compact('data','dictionaries','ticketDetails','total','locations'));
			}
			return redirect()->to('/');
		...
		public function flightDetails(){
          ...
          return redirect()->to('agent/FlightList/Review');
		}
	...
	//	Mail::to($booking->email)->send(new TicketMail($booking));
		 ...
		 return redirect()->to('agent/bookings');
		}
}
?>