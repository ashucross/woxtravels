...
<script>
  gtag('event', 'conversion', {
      'send_to': 'AW-619001147/1uzZCO6e5YEDELvqlKcC',
      'value': 1.0,
      'currency': 'INR'
  });
</script>
...
<div class="name">@if($arriveflighdata[$fla]->Airline->AirlineCode == 'I5')
										...
										<div class="block-content-2 custom_block_content contact_detail">
											<div class="box-result custom_box_result">
												...
												<div class="col-sm-10">
													...
													<div class="form_field country_field"><div class="country_code"><input class="" id="telephone" type="tel" name="telephone" readonly ></div><div class="mobile_no"><input data-valid="required minlength7 maxlength15" value="{{@Auth::user()->phone}}"  id="phone" name="phone" type="tel" placeholder="Mobile Number" class="form-control"/></div></div>
	...
	<div class="promo_code sidebar_bgclr inner_sidebar">
		<div class="inner_promo">
			...
			<?php 
			$today = date('Y-m-d');
				$coupons = \App\Coupon::whereDate('start_date','<=', $today)->whereDate('end_date','>=', $today)->where('status',1)->where('type','flights')->get();
		 ...
		 $(document).ready(function() {
                <?php
					if(isset($_GET['IsReturn']) && $_GET['IsReturn'] != 'False'){
						?>
						$.ajax({
							url: '{{URL::to('/')}}/Flight/FareQuote?ResultIndex={{@$_GET['Index']}}&TraceId={{$_GET['tid']}}&IsInternational={{$_GET['IsInt']}}&IsReturn=true&ResultIndexR={{@$_GET['IndexR']}}&IsLccR=False&price={{$total}}',
							type:'GET',
						...
						console.log('sssd');
						$.ajax({
							url: '{{URL::to('/')}}/Flight/FareQuote?ResultIndex={{@$_GET['RIndex']}}&TraceId={{$_GET['tid']}}&IsInternational={{$_GET['isINT']}}&IsReturn=False&ResultIndexR=&IsLccR=False&price={{$total}}',
							type:'GET',
							success:function(html){
								$('#DivFareQuote').html(html);
								BindMealBaggage();
			 ...
			 function BindMealBaggage() {
				 setTimeout(function () {
					...
					// $(".bagageandmeal").load("");
					  $.ajax({
									url: '{{URL::to('/')}}/Passenger/GetSSR?ResultIndex={{@$_GET['Index']}}&TraceId={{$_GET['tid']}}&IsLcc=True&IsInternational={{$_GET['IsInt']}}&IsReturn=true&ResultIndexR={{@$_GET['IndexR']}}&IsLccR=False',
									type:'GET',
									success:function(html){
			 ...
			 <?php
						...
						setTimeout(function () {
							 $.ajax({
									url: '{{URL::to('/')}}/Passenger/GetSSR?ResultIndex={{@$_GET['RIndex']}}&TraceId={{$_GET['tid']}}&IsLcc=True&IsInternational={{$_GET['isINT']}}&IsReturn=False&ResultIndexR=&IsLccR=False',
									type:'GET',
									success:function(html){
										var obj = JSON.parse(html);
										$('.onward_flight').html(obj.onwardflight);
