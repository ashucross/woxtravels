	...
	<div style="border: 1px solid #89ad3e;padding: 30px;max-width: 1140px;width: 100%;margin: 0px auto;padding-left: 15px;padding-right: 15px;font-family: Verdana, Geneva, Tahoma, sans-serif">
		<div style="background:#f1f1f1;padding: 10px;border:1px solid #ddd;border-radius:10px;">
			<div style="width:40%; float:left; color:#214a93">
				...
				<span style="display:block;"><b style="color:#214a93;">PNR. : </b> {{$booking->Response->Response->PNR}}</span>
			</div>  
			...
			@if(@$Agent->logo != '')
				<img style="width:auto;" src="{{URL::to('/public/img/profile_imgs')}}/{{@$Agent->logo}}">
				@else
			...
			<table style=" border: 1px solid #dee2e6;border-collapse: collapse;" width="100%">
				<tbody>	
					<tr>
						...
						<th style="border: 1px solid #dee2e6; color:#214a93;text-align:center;">Ticket Number</th>
					...
					<tr>
						...
						<td style="border: 1px solid #dee2e6; color:#214a93;text-align:center;">{{$pes[$ps]->Title}} {{$pes[$ps]->FirstName}} {{$pes[$ps]->LastName}} ({{@$paxtype}})
						...
						<td style="border: 1px solid #dee2e6; color:#214a93;text-align:center;">{{@$pes[$ps]->Ticket->TicketNumber}} </td>
		...
		<div style="width:100%;display: flex;flex-wrap: wrap;">
			<div style="background:#f9f9f9;color:#000;border-radius: 5px;max-width: 100%;flex: 0 0 100%;position: relative;min-height: 1px;padding-right: 0px;padding-left: 0px;border-top:3px solid #89ad3e;">
				<span style="font-size:16px;line-height:21px;padding:8px;display: block;">{{$booking->Response->Response->FlightItinerary->Origin}} - {{@$booking->Response->Response->FlightItinerary->Destination}} ({{date('D d/m/Y', strtotime(@$booking->Response->Response->FlightItinerary->Segments[0]->Origin->DepTime))}})</span>
			</div>
		</div>
		<div style="max-width: 100%;flex: 0 0 100%;position: relative;min-height: 1px;padding-right: 15px;padding-left: 15px;">
			<br>
			<table style=" border: 1px solid #dee2e6;border-collapse: collapse;" width="100%">
				<tbody>
					...
					<tr>
						...
						<td style="border: 1px solid #dee2e6; text-align:center;"><small>{{$allflighdata[$fl]->Origin->Airport->AirportName}} ({{$allflighdata[$fl]->Origin->Airport->AirportCode}}) {{$allflighdata[$fl]->Origin->Airport->CityName}}<br> {{date('d/m/Y H:i:s', strtotime($allflighdata[$fl]->Origin->DepTime))}}<br> Terminal  {{$allflighdata[$fl]->Origin->Airport->Terminal}}</small></td>
						<td style="border: 1px solid #dee2e6; text-align:center;"><small>{{$allflighdata[$fl]->Destination->Airport->AirportName}}  ({{$allflighdata[$fl]->Destination->Airport->AirportCode}}) {{$allflighdata[$fl]->Destination->Airport->CityName}}<br> {{date('d/m/Y H:i:s', strtotime($allflighdata[$fl]->Destination->ArrTime))}}<br> Terminal  {{$allflighdata[$fl]->Destination->Airport->Terminal}} </small></td>
				...
				<tfoot>
				<tr>
					<td colspan="5"> Cabin: {{@$booking->Response->Response->FlightItinerary->Segments[0]->CabinBaggage }}| Check-In: {{@$booking->Response->Response->FlightItinerary->Segments[0]->Baggage}}</td>
				</tr>
			</tfoot>
			</table>
		</div>
		...
		<div style="width:100%;display: flex;flex-wrap: wrap;">
			<div style="background:#f9f9f9;color:#000;border-radius: 5px;max-width: 100%;flex: 0 0 100%;position: relative;min-height: 1px;padding-right: 0px;padding-left: 0px;border-top:3px solid #89ad3e;">
				<span style="font-size:16px;line-height:21px;padding:8px;display: block;">{{$bookingib->Response->Response->FlightItinerary->Origin}} - {{@$bookingib->Response->Response->FlightItinerary->Destination}} ({{date('d/m/Y', strtotime(@$bookingib->Response->Response->FlightItinerary->Segments[0]->Origin->DepTime))}})</span>
			</div>
		</div>
		<div style="max-width: 100%;flex: 0 0 100%;position: relative;min-height: 1px;padding-right: 15px;padding-left: 15px;">
			<br>
		...
		<tr>
			...
			<td style="border: 1px solid #dee2e6; text-align:center;"><small>{{$allflighdataib[$flib]->Origin->Airport->AirportName}} ({{$allflighdataib[$flib]->Origin->Airport->AirportCode}})  {{$allflighdataib[$flib]->Origin->Airport->CityName}}<br> {{date('d/m/Y H:i:s', strtotime($allflighdataib[$flib]->Origin->DepTime))}}<br> Terminal  {{$allflighdataib[$flib]->Origin->Airport->Terminal}}</small></td>
			<td style="border: 1px solid #dee2e6; text-align:center;"><small>{{$allflighdataib[$flib]->Destination->Airport->AirportName}} ({{$allflighdataib[$flib]->Destination->Airport->AirportCode}}) {{$allflighdataib[$flib]->Destination->Airport->CityName}}<br> {{date('d/m/Y H:i:s', strtotime($allflighdataib[$flib]->Destination->ArrTime))}}<br> Terminal  {{$allflighdataib[$flib]->Destination->Airport->Terminal}}</small></td>
	...
	<tbody>
		...
		<tr>
			...
			<td style="border: 1px solid #dee2e6; text-align:right;">{{$paymentdta->org_amount}}</td>
					...
					@endif
					@if(@$paymentdta->ins_amount !='')
					<tr>
						<td style="border: 1px solid #dee2e6; text-align:center;">Travel Protection <!--(<a target="_blank" href="{{URL::to('/public/policy')}}/{{@$fetchedData->policy}}.pdf">{{@$fetchedData->policy}}</a>)--></td>
						<td style="border: 1px solid #dee2e6; text-align:right;">{{@$paymentdta->ins_amount}}</td>
					</tr>
					@endif
					<!--<tr>
						...
						<?php $servfee =0; ?>
						@if($paymentdta->service_fee !='')
							<?php
							$servfee = $paymentdta->service_fee;
						?>
							{{@$paymentdta->service_fee}} @else 0 @endif</td>
					</tr>-->
					<tr>
		...
		<br>
		<div style="width:100%;display: flex;flex-wrap: wrap;">
			<div style="max-width: 50%;flex: 0 0 50%;position: relative;min-height: 1px;text-align:center;padding-right: 0px;padding-left: 0px;"><b>Customer E-mail :</b><br> {{@$fetchedData->email}}</div>
			<div style="max-width: 33.333333%;flex: 0 0 50%;position: relative;min-height: 1px;text-align:center;padding-right: 0px;padding-left: 0px;"><b>Customer Contact No. :</b> <br>{{@$fetchedData->mobile}}</div>
		</div>
		<div style="width:100%;display: flex;flex-wrap: wrap;">
			<hr style="border: 1px solid #214a93; width:100%;">
		</div>
		...
		<div style="width:100%;display: flex;flex-wrap: wrap;">
			<div style="max-width: 50%;flex: 0 0 50%;position: relative;min-height: 1px;padding-right: 0px;padding-left: 0px;text-align:center;">
				Call us any time on {{$set->phone}}<br> and we will help you out.
			</div>
			<div style="max-width: 50%;flex: 0 0 50%;position: relative;min-height: 1px;padding-right: 0px;padding-left: 0px;text-align:center;">
