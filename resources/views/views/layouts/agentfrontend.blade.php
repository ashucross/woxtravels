...
<head>	
	...
	<title>Zap Booking: Flight, Hotel, Holidays, Fix Departure Packages</title>
...
.agent_flight_info ul li.price_control{padding:10px 0px;}
.agent_flight_info ul li span.price_offer{display: inline-block;float: right;margin-top: 2px;}
.agent_flight_info ul li span.price_offer input{width: auto;height: auto;margin: 0px;vertical-align: middle;}
.agent_box_result ul li span.offer_value{display: none;margin-top: 8px;}
@media(min-width:601px){
	.oneway_search .agent_box_result ul.list-search-result li.price{font-size: 16px;line-height: 21px;}	
	.agent_box_result ul li.price span.txt_small{font-size: 14px;margin-right: 5px;}
}
@media(min-width:481px) and (max-width:600px){
	.oneway_search .agent_box_result ul.list-search-result li.price, .roundtrip_search .agent_box_result ul.list-search-result li.price{font-size:16px;line-height:21px;}
	.agent_box_result ul li.price span.txt_small{font-size: 12px;margin-right: 3px;}
}
@media(max-width:480px){
	.oneway_search .custom_box_result ul.list-search-result li.price{font-size:11px;line-height:16px;}
...
<div class="searchpop1" style="display: none;"><div id="dvWait"></div></div>
	<div id="full-container">
		<!--Header--> 
			@include('../Elements/Frontend/agentheader')
...
<script src='https://www.google.com/recaptcha/api.js'></script>
		...
		var bookurl = '{{URL::to('/agent/Review/Checkout')}}'; 
		</script>
	...
	$.ajax({
	   ...
	   success:function(data)
	   {
		...
		$('.onewayformsearch').on('click', function(){
			...
			if(flag){
				     window.location.href = "{{URL::to('agent/FlightList/index')}}?srch=" + val1 + "|" + val2 + "&dep=" + onewaystartdate + "&px=" + onewayadult + "-" + onewaychild + "-" + onewayinfants + "&cbn=" + seatclass + "&nt=" + is_non_stop + "&jt=" + journey_type + "&from=" + from + "&to=" + to;
		...
		$('.roundformsearch').on('click', function(){
			...
			if(val1 == ''){
				...
				$("#hfFromDestination").val(val1);
					...
					if(journey_type == 2){
                    window.location.href = "{{URL::to('agent/FlightList/index')}}?srch=" + val1 + "|" + val2 + "&dep="+onewaystartdate+"&ret="+onewayenddate+"&px=" + onewayadult + "-" + onewaychild + "-" + onewayinfants + "&cbn=" + seatclass + "&nt=" + is_non_stop + "&jt=" + journey_type + "&from=" + from + "&to="+to;
					}
			  window.location.href = "{{URL::to('agent/FlightList/index')}}?srch=" + val1 + "|" + val2 + "&dep=" + onewaystartdate  + "&px=" + onewayadult + "-" + onewaychild + "-" + onewayinfants + "&cbn=" + seatclass + "&nt=" + is_non_stop + "&jt=" + journey_type + "&from=" + from + "&to=" + to;
		...
		var _parameterForRecent = "";
			...
			$('.searchpop1').show();
			window.location.href= "{{URL::to('/agent/FlightList/index')}}?srch="+_param+"&px="+noOfAdults+"-"+noOfChild+"-"+noOfInfants+"&cbn="+seatclass+"&nt="+is_non_stop+"&jt="+journey_type;
		}
			...
			<div class="modal-dialog">
				...
				<div class="modal-content">
					<div class="modal-body">
						...
						<div class="fare_btn">
							{{-- <a class="another_flight" href="{{URL::to('/agent/FlightList/index')}}?srch={{$searchdata['srch']}}&px={{$searchdata['px']}}&cbn={{$searchdata['cbn']}}&nt={{$searchdata['nt']}}&jt={{$searchdata['jt']}}"><i class="fa fa-arrow-left"></i> Choose Another Flight</a> --}}
