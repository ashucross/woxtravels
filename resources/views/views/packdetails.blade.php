...
#holder .calendar-table {margin: 0 auto;width: 700px;}

#packcalender{width: 100%;margin: 0px;}
#packcalender .calendar-table {width: 100%;}
#packcalender .calendar-table thead tr.c-weeks th{font-size:11px;line-height:14px;}
#packcalender .calendar-table tr td{padding:3px;font-size:11px;line-height:14px;width: 4	0px;}
#packcalender .calendar-table tr td a.btn_2{padding:5px;font-size:10px;line-height:14px;border-radius: 3px!important;}
#packcalender .calendar-day {width: auto;height: auto;}
.selected {background-color: #eee;}
.outside .date {color: #ccc;}
.timetitle {white-space: nowrap;text-align: right;}

#holder .event{border-top: 1px solid #b2dba1;border-bottom: 1px solid #b2dba1;background-image: linear-gradient(to bottom, #dff0d8 0px, #c8e5bc 100%);background-repeat: repeat-x;color: #3c763d;
border-width: 1px;font-size: .75em;padding: 0 .75em;line-height: 2em;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;margin-bottom: 1px;}

#holder .event.begin{border-left: 1px solid #b2dba1;border-top-left-radius: 4px;border-bottom-left-radius: 4px;}
#holder .event.end {border: 1px solid #89ad3e;border-radius: 0px;background: transparent;margin-bottom: 2px;padding: 3px 5px;font-size: 11px;line-height: 14px;}
#packcalender .event.begin{border-left: 1px solid #b2dba1;border-top-left-radius: 4px;border-bottom-left-radius: 4px;}
#packcalender .event.end {border: 1px solid #89ad3e;border-radius: 0px;background: transparent; margin-bottom: 2px;padding: 0px;font-size: 10px;line-height: 14px;width: 20px;overflow: hidden;}
.event.all-day {
...
#holder table.calendar-table tbody tr td a.btn_2{padding: 5px 10px;font-size: 12px;line-height:14px; border-radius: 4px!important;}
#holder table.calendar-table>thead>tr>td{padding:0px;}
#holder table.calendar-table>thead>tr>td table{border:0px;}
#holder table.calendar-table>thead>tr>td>table>tbody>tr>td:nth-child(1) span.btn-group button, #holder table.calendar-table>thead>tr>td table tr td button.js-cal-option.active{background: #5091fa;border: 0px;color: #fff;padding: 6px 8px;border-radius: 4px;}
#holder table.calendar-table>thead>tr>td table tbody tr td span.btn-group  button:hover{ text-decoration:none;}
#holder table.calendar-table>thead>tr>td>table>tbody>tr>td:nth-child(2) span.btn-group>button{ font-size: 18px;}

#holder table.calendar-table>thead>tr>td>table>tbody>tr>td:nth-child(2) span.btn-group>button, #packcalender table.calendar-table>thead>tr>td>table>tbody>tr>td:nth-child(2) span.btn-group>button {color: #5091fa;padding: 0px;border: 0px;background: transparent;}
#holder table.calendar-table>thead>tr>td>table>tbody>tr>td:nth-child(2) span.btn-group button.js-cal-years{margin-left: 10px;}

#holder table.calendar-table thead tr td table tbody tr td span.btn-group .popover .popover-content button{padding: 5px 10px;font-size: 13px;line-height: 21px;height: auto}
#holder table.calendar-table thead tr.c-weeks th.c-name{background: #5091fa;color: #fff;
font-size: 13px;line-height: 21px;}
#holder table.calendar-table thead tr.c-weeks th.c-name, #holder table.calendar-table tbody td.calendar-day{padding-left:5px!important;padding-right: 5px !important;}
#holder table.calendar-table tbody td.calendar-day .popover{display:none!important;}
#holder table.calendar-table thead tr td table tr td{padding:10px;}

#packcalender table.calendar-table>thead>tr>td>table>tbody>tr>td:nth-child(1) span.btn-group button, #packcalender table.calendar-table>thead>tr>td table tr td button.js-cal-option.active{background: #5091fa;border: 0px;color: #fff;padding: 4px 6px;border-radius: 4px;}
...
<section class="main_detail_pack"> 
		...
		<div class="container">
			<div class="pack_banner_title">
				<div class="inner_title">
					...
					<p><i class="fa fa-map-marker"></i> {{@$dest->details_day_night}}</p>
				</div>				
				<!--<div class="pack_price"> 
					<span class="banner_code_span">Tour Code: <strong>{{@$dest->tour_code}}</strong></span>	
				 @if(@$dest->price_on_request == 1)
						<strong>Price On Request</strong>
					@else	 
						<span>Price:</span>
						<div class="price_val"><?php /*<strike><strong><i class="fa fa-rupee-sign"></i> {{@$dest->data->package_detail->sales_price}}</strong></strike>*/ ?>
						<span class="actual_price"><i class="fa fa-rupee-sign"></i> {{$dest->sales_price}}</span></div>
				@endif 
...
<div class="container">
	<div class="row">
		<div class="col-sm-12">
			<!--<div class="trip_time">
				<h5>Chandigarh <span>(2N)</span> <i class="fa fa-angle-right"></i> Monali <span>(1N)</span> <i class="fa fa-angle-right"></i> Kasauli <span>(1N)</span></h5>
				<h5><?php //echo @$dest->details_day_night; ?></h5>
			</div>-->
		</div>
	</div>
</div>
<div class="details_main">
			...
			<div id="row_scroll" class="row" style="transform: none;">
				<div class="col-lg-9">
					<section class="common_section">
						<div class="row pack_disflex">
							<div class="col-sm-8 slidersection gallery_section flex_col">
					...
					<section id="inclu_exclu" class="common_section">						
						<div class="row">
							<div class="col-lg-6 brder_rgt">
								...
								<ul class="bullets">
									<?php 
										$explodeei = explode(',',@$dest->package_inclusions);
							...
							<div class="col-lg-6">
								...
								<ul class="bullets">
								<?php 
									$explodeee = explode(',',@$dest->package_exclusions);
										...
										<div class="row row_margin_5">
											<div class="col-sm-8 padding5px">
												<div class="inner_hotel_pack"> 
													...
													?>
														<span class="textblack13bold ng-scope" ng-if="hotel.Locality!=''">City:</span><span ng-if="hotel.Locality!=''" class="textblue13 optionalCategory ng-binding ng-scope" ng-bind-html="hotel.Locality | rawHtml"> {{@$cityname->name}}</span><!-- end ngIf: hotel.Locality!='' -->
													</div>
												</div>
											</div>
	...
	<div class="box-result custom_box_result">
		<ul class="list-search-result result_list">
			...
			<li>
				<span class="duration"><?php echo Controller::GetTimeduration($depflight->dep_time, $depflight->arival_time); ?>
...
<div role="tabpanel" class="tab-pane active" id="flightinfo00">
	...
	<div class="flight_route">
		...
		<span class="flight_no">{{$depflight->flight_number}}</span></div>
		</li>
	...
	{{@$depflight->flightdest->city_code}} {{date('h:i', strtotime($depflight->arrival_time))}} <span>{{date('D, d M Y', strtotime($depflight->arival_time))}}</span>
		...
		<div role="tabpanel" class="tab-pane" id="faredetail01">
			<div class="fare_details">
				<div class="row">
					<p><?php echo htmlspecialchars_decode(stripslashes(@$depflight->fare_detail)); ?></p>	<div class="clearfix"></div>
				</div>
			</div>
		</div>
	...
	<?php echo htmlspecialchars_decode(stripslashes(@$depflight->cancellation_policy)); ?><div class="clearfix"></div> 
	</div>
	</div>
				</div>
			</div> 
...
<?php
	...
	<div class="box-result custom_box_result">
		<ul class="list-search-result result_list">
			...
			<li>
				<span class="duration"><?php echo Controller::GetTimeduration($retflight->dep_time, $retflight->arival_time); ?>
...
<div role="tabpanel" class="tab-pane active" id="flightinfo01">
	...
	<div class="flight_route">
		...
		<img src="{{URL::to('/public/img/airline')}}/{{@strtoupper($retflight->flight->code)}}.gif" alt="">
		<div class="flight_name">
		<span class="flight_no">{{$retflight->flight_number}}</span></div>
		</li>
		<li class="flight_timer">
	...
	{{@$retflight->flightdest->city_code}} {{date('h:i', strtotime($retflight->arrival_time))}} <span>{{date('D, d M Y', strtotime($retflight->arival_time))}}</span>
		...
		<div role="tabpanel" class="tab-pane" id="faredetail02">
			<div class="fare_details">
				<div class="row">
					<p><?php echo htmlspecialchars_decode(stripslashes(@$retflight->fare_detail)); ?></p>	<div class="clearfix"></div>
				</div>
			</div>
		</div>
	...
	<div class="cancellationrule_info">
	<?php echo htmlspecialchars_decode(stripslashes(@$retflight->cancellation_policy)); ?><div class="clearfix"></div> 
	</div>
	</div>
				</div>
			</div> 
...
@endif
			...
			<section id="terms" class="common_section">
				...
				<?php echo htmlspecialchars_decode(stripslashes(@$dest->package_tourpolicy)); ?>
				</article>
			</section>
					<!-- /section -->
				</div>
...
<input type="hidden" class="mytotalprice" value="0">							
								<div class="pack_price_txt">
									...
									<br>
									<sub>Price Per Adult Twin Sharing Basis</sub></h4>
								</div>  
							</div>  
							
							<div class="pack_book_btn"> 
								<a href="javascript:void(0)" id="holidays-booking" class="">Book Online</a>
						...
						<div class="item">
							<div class="box_grid">
								<figure>
									...
									<a href="{{URL::to('/destinations/'.$dslug.'/'.$rplist->slug)}}"><img src="{{URL::to('/public/img/media_gallery')}}/{{@$rplist->media->images}}" class="img-fluid" alt="" width="800" height="533"><div class="read_more"><span>Read more</span></div></a>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Booking Enquiry</h5>
      ...
      <div class="modal-footer">
        ...
        <button type="submit" class="btn btn-primary">Book Enquiry</button>
      </div>
	    </form>
    </div>
  </div>
...
$expss =  explode(',',$slpriced->departure_date);
...
var data = [],
    ...
    m;
<?php
foreach($pricesmy as $lpriced){
$exp =  explode(',',$lpriced->departure_date);
$no_of_seats = json_decode($lpriced->no_of_seats);	
...
?> 
data.push({ seats:'<?php echo $no_of_seats[$key]; ?>',title: '<?php echo @$lpriced->twin; ?>', start: new Date('<?php echo $year[2]; ?>', '<?php echo $year[0] -1; ?>', '<?php echo $year[1]; ?>', '00', '00'), end: new Date('<?php echo $year[2]; ?>', '<?php echo $year[0] -1; ?>', '<?php echo $year[1]; ?>', '00', '00'), allDay: 0, text: '', href:'javascript:;', datename:'<?php echo $mdate; ?>', datetimem:'<?php echo strtotime($date); ?>'  });
<?php } } } ?>
  data.sort(function(a,b) { return (+a.start) - (+b.start); });
//https://zapbooking.com/package/booking/{{base64_encode(convert_uuencode(@$dest->id))}}?date={{@$date}}
</script>
<script>  
$("#booking-details").submit(function(stay){
	 $('.errors').remove();
			...
			console.log(response.status);
			 if (response.status === 401) {
                      $('#booking-modal').modal('hide');
						  ...
						  localStorage.setItem('booking-details', formdata);
...
$('#holidays-booking').on('click',function(){
		...
		$('#login-form').submit(function(stay){
		
			   
				$('.loading-div').css('display','block');
        var formdat = $(this).serialize(); // here $(this) refere to the form its submitting
       $.ajax({
        type: 'POST',
        url: "{{ url('ajax/login') }}",
        data: formdat, // here $(this) refers to the ajax object not form
        success: function (data) {
			$('.loading-div').css('display','none');
          $('#login-modal').modal('hide');
		  var formdatas = localStorage.getItem('booking-details');
		    $("#login-form")[0].reset();
		  booking($.trim(formdatas))
        },
		error:function(response){
			$('.loading-div').css('display','none');
					if (response.status === 400) {
					    $('#login-modal-message').after('<span class="errors">' + value + '</span>')
                    }
                    if (response.status === 422) {
                        var errors = $.parseJSON(response.responseText);
                        $.each(errors.errors, function(key, value) {
	...
	$(".custom-error").remove(); 
		var formName = 'personpackage';
			$("form[name="+formName+"] :input[data-valid]").each(function(){
				console.log((this).attr('data-valid'));
