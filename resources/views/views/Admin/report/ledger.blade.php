...
<ul>
@if(isset($_GET['ref']))
<li class="export"><a href="{{URL::to('/report/ledger')}}?ref={{@$_GET['ref']}}&type=excel"><i class="fas fa-file-excel"></i> Excel Export</a></li>
<li class="export"><a href="{{URL::to('/report/ledger')}}?ref={{@$_GET['ref']}}&type=pdf"><i class="fas fa-file-excel"></i> PDF Report</a></li>
<li class="print"><a href="javascript:;" dataurl="{{URL::to('/report/ledger')}}?ref={{@$_GET['ref']}}&type=print" class="print_myinvoice"><i class="fas fa-print"></i> Print</a></li>
@else 
	<li class="export"><a href="{{URL::to('/report/ledger')}}?agent={{@$_GET['agent']}}&submission_start={{@$_GET['submission_start']}}&submission_end={{@$_GET['submission_end']}}&type=excel"><i class="fas fa-file-excel"></i> Excel Export</a></li>
<li class="export"><a href="{{URL::to('/report/ledger')}}?agent={{@$_GET['agent']}}&submission_start={{@$_GET['submission_start']}}&submission_end={{@$_GET['submission_end']}}&type=pdf"><i class="fas fa-file-excel"></i> PDF Report</a></li>
<li class="print"><a class="print_myinvoice" href="javascript:;" dataurl="{{URL::to('/report/ledger')}}?agent={{@$_GET['agent']}}&submission_start={{@$_GET['submission_start']}}&submission_end={{@$_GET['submission_end']}}&type=print"><i class="fas fa-print"></i> Print</a></li>
@endif
</ul>
</div>
<div class="clearfix"></div>
<div class="search_ref_id">	
<form action="{{route('admin.ledger')}}" method="get">
<div class="row">	
<div class="col-sm-4">
	...
	<div class="col-sm-4">
		<div class="form-group"> 
			...
			<select required class="form-control" name="agent">
				...
				<option value="{{$list->id}}" @if(Request::get('agent') == $list->id) selected @endif>{{$list->username}}</option>
				@endforeach
			</select>
		</div>
	</div>
