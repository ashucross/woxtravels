				...
				<div class="col-md-12">
					<div class="card">
						<div class="card-header">
							...
							?>
									<li style="cursor:pointer;" onClick="window.location.href='{{URL::to('/bookings/?status=1')}}'" class="active_tag cus_tag">
										<span class="span_tag approvetag">{{@$suucessb}}</span> <span class="tag_label">Success
										<small class="approvetag"> {{@$suucessb}}</small></span>
										
									</li>
									<li style="cursor:pointer;"  onClick="window.location.href='{{URL::to('/bookings/?status=2')}}'" class="inactive_tag cus_tag">
										<span class="span_tag pendingtag">{{@$failedb}}</span> <span class="tag_label">Failed 
										<small class="pendingtag"> {{@$failedb}}</small></span>
									</li>
									<li style="cursor:pointer;"  onClick="window.location.href='{{URL::to('/bookings/?status=0')}}'" class="inactive_tag cus_tag">
					...
					<div class="card"> 
						<div class="card-header">   
								...
								<ul>
									<li> 
										...
										<div class="dropdown-menu" x-placement="bottom-start" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(0px, 31px, 0px);">
										  <a class="dropdown-item" tabindex="-1" href="{{URL::to('/bookings')}}">B2C Booking</a>
										  <a class="dropdown-item" tabindex="-1" href="{{URL::to('/bookings')}}?type=b2b">B2B Booking</a>
										</div>											
									</li>
								</ul>
							</div>
							...
							<div class="card_custom_menu">
								<ul>
									<li class="<?php if($bstype == 'flight'){ echo 'active'; } ?>"><a href="{{URL::to('/bookings')}}?type={{$ty}}">Flight</a></li>
									<li class="<?php if($bstype == 'hotel'){ echo 'active'; } ?>"><a href="{{URL::to('/bookings')}}?btype=hotel&type={{$ty}}">Hotel</a></li>
									<li class="<?php if($bstype == 'package'){ echo 'active'; } ?>"><a href="{{URL::to('/bookings')}}?btype=package&type={{$ty}}">Package</a></li>
							...
							<div class="print_export">
								<ul>
		<li class="print"><a dataurl="{{URL::to('/bookings')}}?type={{@$_GET['type']}}&action=print&pnr={{@$_GET['pnr']}}&ref={{@$_GET['ref']}}&source={{@$_GET['source']}}&destination={{@$_GET['destination']}}&email={{@$_GET['email']}}&mobile={{@$_GET['mobile']}}&status={{@$_GET['status']}}" href="javascript:;" class="print_myinvoice"><i class="fas fa-print"></i> Print</a></li>
									<li class="export"><a href="{{URL::to('/bookings')}}?type={{@$_GET['type']}}&action=excel&pnr={{@$_GET['pnr']}}&ref={{@$_GET['ref']}}&source={{@$_GET['source']}}&destination={{@$_GET['destination']}}&email={{@$_GET['email']}}&mobile={{@$_GET['mobile']}}&status={{@$_GET['status']}}"><i class="fas fa-file-excel"></i> Export</a></li>
								</ul>
							</div>
							...
							<table id="invoicetable" class="table table-bordered table-hover text-nowrap">
								...
								?>
									<tr>
										...
										<td id="">
											    @if($bookdetail->booking_response)
												@php $pass= getpassengerDetails($bookdetail->booking_response)		 @endphp
												@if($pass)
												@foreach($pass as $pas)
												  {{$pas['fullname']}} ({{$pas['type']}})<br>
										...
										<td>
											<div class="check_status">
												...
												<span><a href="{{URL::to('/log/'.base64_encode(convert_uuencode(@$bookdetail->id)))}}" target="_blank" class="view_log">View Log</a></span>
											</div>
										</td>
										<td>
											<div class="check_status">
										...
										<td>
											<div class="nav-item dropdown action_dropdown">
												...
												<div class="dropdown-menu">
													<a href="{{URL::to('/bookings/detail/'.base64_encode(convert_uuencode(@$bookdetail->id)))}}"><i class="fa fa-edit"></i> View Detail</a>
...
<div class="modal fade" id="amnetsearch_modal">
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			...
			<form action="{{URL::to('/bookings')}}" method="get">
