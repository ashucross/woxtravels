						...
						<div class="card-header">   
							...
							<div class="print_export" style="margin-top: 13px;">
								<ul>
									...
									<li class="export"><a href="{{URL::to('/excel_agents_log')}}?agent_id={{@$_GET['agent_id']}}&company_name={{@$_GET['company_name']}}&from={{@$_GET['from']}}&to={{@$_GET['to']}}&email={{@$_GET['email']}}&mobile={{@$_GET['mobile']}}&status={{@$_GET['status']}}"><i class="fas fa-file-excel"></i> Export</a></li>
								</ul>
							</div>
						</div>  
						...
						<div class="card-body table-responsive p-0">
							<table id="departurecity_table" class="table table-bordered table-hover text-nowrap">
							  ...
							  <tbody class="tdata">	 
								...
								<tr id="id_{{@$list->id}}"> 
									<td><a href="{{URL::to('/agents/view/'.base64_encode(convert_uuencode(@$list->id)))}}">{{$list->username}}</a></td>
								  ...
								  <td>
									<div class="nav-item dropdown action_dropdown cus_action_btn">
										...
										<div class="dropdown-menu">
											<a href="{{URL::to('/agents/view/'.base64_encode(convert_uuencode(@$list->id)))}}"><i class="fa fa-eye"></i> View Detail</a>
											<a href="{{URL::to('/agents/send-password/'.base64_encode(convert_uuencode(@$list->id)))}}"><i class="fa fa-eye-slash"></i> Send Password</a>  
											<a target="_blank" href="{{URL::to('/agents/agentlogin')}}/{{base64_encode(convert_uuencode(@$list->id))}}"><i class="fa fa-sign-in"></i> Login</a>
										</div>
									</div> 
									<div class="nav-item dropdown action_dropdown cus_action_btn">
										...
										<div class="dropdown-menu">
											<a href="{{URL::to('/wallet/create')}}?agent_id={{base64_encode(convert_uuencode(@$list->id))}}&type=topup"><i class="fa fa-money"></i> TopUP Amount</a>
											<a href="{{URL::to('/wallet/create')}}?agent_id={{base64_encode(convert_uuencode(@$list->id))}}&type=deduct"><i class="fa fa-money"></i> Deduct Amount</a>
											<a href="javascript:;" data-id="{{@$list->id}}" class="setcreditlimit"><i class="fa fa-credit-card"></i> Set Credit Limit</a>
											<a href="{{URL::to('/agents/transactionlog/'.base64_encode(convert_uuencode(@$list->id)))}}"><i class="fa fa-money"></i> Transaction Log</a>
											<a href="{{URL::to('/agents/credit_limit_log/'.base64_encode(convert_uuencode(@$list->id)))}}"><i class="fa fa-credit-card"></i> Credit Limit Log</a>
										</div>  
									</div> 	
								  </td> 
								</tr>	
...
jQuery(document).ready(function($){
	...
	$(document).delegate('.setcreditlimit', 'click', function(){
		...
		$.ajax({
			type:'get',
			url:'{{URL::to('agents/setlimit/')}}/'+vid,
			processData: false,
			contentType: false,
