									  ...
									  <td>
										<div class="nav-item dropdown action_dropdown">
											...
											<div class="dropdown-menu">
											  <a href="{{URL::to('/hotel/edit/'.base64_encode(convert_uuencode(@$list->id)))}}"><i class="fa fa-edit"></i> Edit</a>
											  <a href="{{URL::to('/hotel/edit/'.base64_encode(convert_uuencode(@$list->id)))}}"><i class="fa fa-eye"></i> View</a>
								...
								?>
									<select class="form-control" name="destination">
										...
										@foreach(\App\Destination::where('user_id', '=', Auth::user()->id)->with([ 'myloc'])->get() as $dlist)
											<option value="{{$dlist->dest_id}}" @if($desttype == $dlist->dest_id) selected @endif>{{$dlist->myloc->name}}</option>
										@endforeach
									</select>
								</div>
							</div>
