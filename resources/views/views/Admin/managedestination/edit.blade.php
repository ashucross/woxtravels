...
@section('title', 'Edit Manage Destination')
				...
				<div class="col-md-12">
					<div class="card card-primary">
					  <div class="card-header">
						<h3 class="card-title">Edit Manage Destination</h3>
					  </div>
					  <!-- /.card-header -->
					  <!-- form start -->
					  {{ Form::open(array('url' => 'destination/edit', 'name'=>"edit-destination", 'autocomplete'=>'off', "enctype"=>"multipart/form-data")) }}
					  {{ Form::hidden('id', @$fetchedData->id) }}
						<div class="card-body">
							<div class="form-group" style="text-align:right;">
								<a style="margin-right:5px;" href="{{route('admin.managedestination.index')}}" class="btn btn-primary"><i class="fa fa-arrow-left"></i> Back</a>	
								{{ Form::button('<i class="fa fa-edit"></i> Update Destination', ['class'=>'btn btn-primary', 'onClick'=>'customValidate("edit-destination")' ]) }}
							</div> 
						...
						<div class="form-group row"> 
								...
								<div class="col-sm-10">
									<select name="destination" data-valid="required" id="destination" class="form-control" autocomplete="new-password">
										<option value="">-- Select Destination --</option>
												...
												<option value="{{ @$Locatio->id }}" @if(@$fetchedData->dest_id == $Locatio->id) selected  @endif  >{{ @$Locatio->name }}</option>
												@endforeach
						  ...
						  {{--  <div class="form-group row"> 
								<label for="dest_name" class="col-sm-2 col-form-label">Dest. Name <span style="color:#ff0000;">*</span></label>
								<div class="col-sm-10">
								{{ Form::text('dest_name', @$fetchedData->dest_name, array('class' => 'form-control', 'data-valid'=>'required', 'autocomplete'=>'off','placeholder'=>'Enter Dest Name' )) }}
								@if ($errors->has('dest_name'))
									<span class="custom-error" role="alert">
										<strong>{{ @$errors->first('dest_name') }}</strong>
									</span> 
								@endif
								</div>
						  </div> --}}
