...
class HotelbookingController extends Controller
{
	...
	public function index(Request $request){
		...
		curl_setopt_array($curl, array(
		  CURLOPT_URL => 'https://api-sandbox.grnconnect.com/api/v3/hotels/availability/'.$sid.'?hcode='.$hid,
		  ...
		  CURLOPT_HTTPHEADER => array(
			'api-key: 1a6e0a4e04542450662cdde740bee3dc',
		...
		//echo '<pre>'; print_r($detail); die;
			...
			return view('hotel-booking', compact('detail','rm')); 
		}
		 }
	} 
	...
	public function HotelPayment(Request $request){
		...
		curl_setopt_array($curl, array(
		  CURLOPT_URL => 'https://api-sandbox.grnconnect.com/api/v3/hotels/availability/'.$searchid.'?hcode='.$hotel_code,
		  CURLOPT_RETURNTRANSFER => true,
		  CURLOPT_ENCODING => '',
		  CURLOPT_MAXREDIRS => 10,
		  CURLOPT_TIMEOUT => 0,
		...
		// echo '<pre>'; print_r($detail); die;
		 if(isset($detail->hotel->rates[$rm])){
			 ...
			 if($rooms->rate_type == 'recheck'){
			  $rcurl = curl_init();
				$ratepost['rate_key'] = $rate_key;
				$ratepost['group_code'] = $group_code;
		
				curl_setopt_array($rcurl, array(
			  CURLOPT_URL => 'https://api-sandbox.grnconnect.com/api/v3/hotels/availability/'.$searchid.'/rates/?action=recheck',
			   CURLOPT_RETURNTRANSFER => true,
			  CURLOPT_ENCODING => '',
			  CURLOPT_MAXREDIRS => 10,
			  CURLOPT_TIMEOUT => 0,
		...
		$hotellog->info('hotelrecheckrequest', $log);
			$ratedetail =  json_decode($responserate);
			// echo '<pre>'; print_r($ratedetail); die;
		 }
			 
			$finaltotal = $detail->hotel->rates[$rm]->price;
	...
	//	echo '<pre>'; print_r($booking_request['adulttitle']); die;
			 ...
			 $hotelreq['booking_name'] = 'Test Booking';
			 $hotelreq['group_code'] = $group_code;
			 $hotelreq['checkin'] = $checkin;
			 $hotelreq['checkout'] = $checkout;
			 $hotelreq['payment_type'] = "AT_WEB";
			 $hotelreq['agent_reference'] = "agent test";
			 $hotelreq['holder'] = array(
				'title' => $adulttitle[1][0],
				'name' => $firstname[1][0],
				'surname' => $lastname[1][0],
				'email' =>	$email,
				'phone_number' =>	$mobile,
			...
			if(isset($detail->hotel->rates[$rm])){
			 ...
			 if(isset($detail->hotel->rates[$rm]) || isset($detail->hotel->rate)){
				...
				$allrooms = array();
				$i=0;
				foreach($detail->hotel->rates as $keys => $rates){
					$is = 1;
				if($i<=1){		
				foreach($rates->rooms as $key => $room){
					$paxes = array();
					$rmss = array();
...
$log = ['request' => $hotelpost,'response' => $hotellists];
		...
		$hotellog->info('hotelnewbooking', $log);
			...
			curl_setopt_array($curlss, array(
			  CURLOPT_URL => 'https://api-sandbox.grnconnect.com/api/v3/hotels/bookings/'.$data->booking_reference.'?type=GRN',
			  CURLOPT_RETURNTRANSFER => true,
			  CURLOPT_ENCODING => '',
			  CURLOPT_MAXREDIRS => 10,
			  CURLOPT_TIMEOUT => 0,
...
$log = ['request' => $hotelpost,'response' => $responsess];
	...
	public function HotelBookingSuccess(Request $request, $id = Null){
		if(isset($id) && !empty($id)){
			...
			curl_setopt_array($curlss, array(
			  CURLOPT_URL => 'https://api-sandbox.grnconnect.com/api/v3/hotels/bookings/'.$fetchedData->booking_reference.'?type=GRN',
			  CURLOPT_RETURNTRANSFER => true,
			  CURLOPT_ENCODING => '',
			  CURLOPT_MAXREDIRS => 10,
			  CURLOPT_TIMEOUT => 0,
			...
			//echo '<pre>'; print_r($detail); echo '<pre>'; die;
				...
				{
					//return Redirect::to('/contact')->with('error', 'Contact Not Exist');
				}
		}
